...
public class RegisterActivity extends Activity {
	private Button register, getCode; // 注册、获取验证码按钮
			...
			if (msg.what == -9) {
				...
				if (result == SMSSDK.RESULT_COMPLETE) {
					...
					if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) {// 提交验证码成功
						...
						if (manager.addUser(user) > 0) {// 插入成功
							CommonFunction.toastShow("注册成功",
									RegisterActivity.this);
							goLogin();
						} else {// 插入失败
							CommonFunction.toastShow("注册失败,请联系数据库管理员稍后再试!",
									RegisterActivity.this);
						}
		...
		getCode.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (CommonFunction.checkPhoneNum(uId)) {
					// 检测网络状态
					if (!NetHelper.checkNetWork(RegisterActivity.this)) {// 没有连接网络
						return;
					}
					// 检测此手机号是否存在
					if (CommonFunction.userIsExistOrNot(new User(uId),
							RegisterActivity.this)) {
						CommonFunction.toastShow("此手机号码已被注册!",
								RegisterActivity.this);
					} else {
						// 通过sdk发送短信验证
		...
		register.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View arg0) {
				...
				if (!"".equals(uId) && !"".equals(pwd) && !"".equals(code)) {
					...
					CommonFunction
							.toastShow("请将信息填写完整!", RegisterActivity.this);
				}
			}
		});
	}
	...
	private void hideKeyBoard() {
		RelativeLayout rl = (RelativeLayout) findViewById(R.id.layout_register);
